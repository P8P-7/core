...
follow_line_detector::follow_line_detector(const cv::Mat& input, int boxes, int box_height, int boxes_bottom_padding,
                                           int boxes_horizontal_padding)
        : detector(input), boxes(boxes), box_height(box_height), boxes_bottom_padding(boxes_bottom_padding),
          boxes_horizontal_padding(boxes_horizontal_padding) {
    if(boxes > 4 || boxes < 0) {
        throw vision_error("The max amount of ROI boxes must be between 1 and 4");
    }
}

follow_line_detector::follow_line_detector(const follow_line_detector& other, int boxes, int box_height,
                                           int boxes_bottom_padding, int boxes_horizontal_padding)
        : detector(other.input), boxes(other.boxes), box_height(other.box_height),
          boxes_bottom_padding(other.boxes_bottom_padding), boxes_horizontal_padding(other.boxes_horizontal_padding) {
}
...
std::vector<cv::Vec4d> follow_line_detector::detect() const {
    ...
    for(int box_index = 0; box_index < boxes; ++box_index) {
        const int x = boxes_horizontal_padding;
        const int y = boxes_bottom_padding + box_height * (box_index + 1);
        const int w = (input.cols - 1) - (boxes_horizontal_padding * 2);
